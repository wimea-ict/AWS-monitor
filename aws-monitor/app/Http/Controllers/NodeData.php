...
class NodeData extends Controller
{
    ...
    {
        $this->middleware(['permission:node-data']);
        $this->middleware(['permission:two-meter'])->only('twometerdata');
        $this->middleware(['permission:ten-meter'])->only('tenmeterdata');
        $this->middleware(['permission:ground-node'])->only('grounddata');
    }
