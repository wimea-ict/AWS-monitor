...
def retrieveStatus(station_id,classificationid,Node,Value):
  if classificationid == '5' or classificationid == '10' or classificationid == '8' or classificationid == '6':
    Sensor=Value.split()
    cursor = mydb.cursor()
    sqlStatement = " select status, id, when_reported, Value from problems where (NodeType='"+Node+"' and `Value`LIKE '%"+Sensor[0]+"%' and stationID='"+str(station_id)+"' AND classification_id = '"+classificationid+"' and status != 'fixed') ORDER BY id  DESC"
    cursor.execute(sqlStatement)
    result = cursor.fetchall()
    return result
  else:
    cursor = mydb.cursor()
    sqlStatement = " select status, id, when_reported, Value from problems where (NodeType='"+Node+"' and stationID='"+str(station_id)+"' AND classification_id = '"+classificationid+"' AND status != 'fixed') ORDER BY id  DESC"
    cursor.execute(sqlStatement)
    result = cursor.fetchall()
    return result


def retrieveStatus_orig(station_id,classificationid,Node):
  cursor = mydb.cursor()
  sqlStatement = " select status, id, when_reported, Value from problems where (NodeType='"+Node+"' and stationID='"+str(station_id)+"') AND (classification_id = '"+classificationid+"' and status != 'fixed') ORDER BY id  DESC"
  cursor.execute(sqlStatement)
  result = cursor.fetchall()
...
def updateProblem(status, entryID):
  ...
  sql = "UPDATE problems SET status = '"+status+"', when_fixed = '"+str(timenow)+"'  WHERE id = " + str(entryID)
