    ...
    if (line.trim()) {
      ...
      line.split(' ').map((item) => {
        ...
        if (item) {
          ...
          if (item.includes('P')) {
            if (item.split('=')[0].length === 1) {
              masterObject.P = item.split('=')[1];
            }
          }
          if (item.includes('V_A1')) {
            masterObject.V_A1 = item.split('=')[1];
          }
          if (item.includes('windSpeed')) {
            masterObject.windSpeed = item.split('=')[1];
          }
...
//handling Relative Humidity for new firmware with RH
          if (item.includes('RH')) {
            masterObject.RH_SHT2X = item.split('=')[1];
          }
    ...
    // we bend the alogorithm to accomodate byd-1 and byd-2
      if (masterObject.NAME.split('-')[0] === 'byd'){
        var station_no = masterObject.NAME.split('-')[1].split('')[0];
        var name = 'byd';
        masterObject.stationname = `${name}-${station_no}`;
      }

      //console.log(masterObject.NAME.split('-')[0]);
      //this line is for normal stations with normal names aside for byd
      if (masterObject.NAME && masterObject.NAME.includes('-') && !(masterObject.NAME.split('-')[0] === 'byd')) {
        if (masterObject.NAME.split('-')[0]==='fosgen3'){
          masterObject.stationname = 'fos';
        }else{
        ...
        var stationNumber2 = masterObject.NAME.split('_')[1].split('')[0];
...
//here
      ...
      //console.log(masterObjectCopy.stationname)
      //console.log(masterObject.stationname)
